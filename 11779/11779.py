...
"https://www.acmicpc.net/problem/11779 최소비용 구하기 2 <Gold III>"
...
def dijkstra(graph, start):
    ...
    routes = [[] for _ in range(num_of_vertex)]

    queue.append((0, start, [start]))
    distances[start] = 0
    
    while queue:
        cost, cur, route = heappop(queue)
        ...
        for next, weight in zip(vertex_list[cur], weight_list[cur]):
            ...
            if predict_distance < next_distance:
                ...
                routes[next] = next_route
                heappush(queue,(predict_distance, next, next_route))
...
distances, routes = dijkstra(graph, start)

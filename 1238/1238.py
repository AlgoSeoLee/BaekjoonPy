...
"https://www.acmicpc.net/problem/1238 파티 <Gold III>"
...
num_of_vertex, num_of_line, target = map(int,stdin.readline().split())
target -= 1
graph = WeightedGraph(num_of_vertex)

for _ in range(num_of_line):
    src, dest, cost = map(int, stdin.readline().split())
    graph.add_line(src, dest, cost)

distances = [0 for i in range(num_of_vertex)]
for i in range(num_of_vertex):
    if i != target:
        travel = dijkstra(graph, i)
        distances[i] = travel[target]

for i, d in zip(range(num_of_vertex), dijkstra(graph, target)):
    distances[i] += d

...
"https://www.acmicpc.net/problem/14938 서강그라운드 <Gold IV>"
...
consts_line = map(int,stdin.readline().split())
num_of_vertex, area_limit, num_of_line = consts_line
graph = WeightedGraph(num_of_vertex, True)

array_of_item_num = [int(v) for v in stdin.readline().split()]
...
for i in range(num_of_vertex):
    finding = dijkstra(graph, i)
    finding_item = 0
    for area, distance in zip(range(num_of_vertex), finding):
        if distance <= area_limit:
            finding_item += array_of_item_num[area]

    if finding_item > result:
        result = finding_item
